...
router.get('/me', auth, asyncTemplate(async (req, res) => {
    const user = await User.findById({ _id: req.token._id }).select('-password');
    if (!user ) return res.status(404).send('No user with matching id found.');
    res.send(user);
}));

//post
router.post('/', validateRequest(validate), asyncTemplate(async (req, res) => {
...
router.put('/:id', [auth, validateRequest(validate), validateObjectId], asyncTemplate(async (req, res) => {
    ...
    await user.set(_.pick(req.body, ['name','email','password', 'isAdmin'])).save();
